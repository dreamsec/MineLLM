    ...
    >
      <el-form label-width="80px">
        <el-form-item label="报表类型">
          <el-radio-group v-model="reportPeriodType" size="small">
            <el-radio-button
              v-for="option in REPORT_PERIOD_OPTIONS"
              :key="option.value"
              :label="option.value"
            >
              {{ option.label }}
        ...
        <el-form-item :label="currentReportMeta.dateLabel">
          <el-date-picker
            v-model="reportDate"
            type="date"
            :placeholder="currentReportMeta.datePlaceholder"
            format="YYYY-MM-DD"
...
const reportDate = ref(getDefaultReportDate(reportPeriodType.value))
...
watch(reportPeriodType, (type) => {
  // 切换报表类型时自动带出对应周期的默认起始日期。
  reportDate.value = getDefaultReportDate(type)
})

watch(selectedType, () => {
  // 切换设备类型后清空旧设备编号，避免把其他类型的编号传给后端。
  selectedCode.value = ''
})

const disabledReportDate = (date: Date) => {
  if (reportPeriodType.value === 'weekly') return date.getDay() !== 1
  if (reportPeriodType.value === 'monthly') return date.getDate() !== 1
  return false
}

async function handleReportDownload() {
  if (!reportDate.value) {
    ElMessage.warning(`请选择${currentReportMeta.value.dateLabel}`)
    return
  }
