...
const parameterDict: Record<string, { label: string, value: string }[]> = {
  hoist: [
    // 模拟量核心运行数据
    { value: 'main_skip_speed', label: '主箕斗提升速度' },
    { value: 'main_skip_pos', label: '主箕斗提升位置' },
    { value: 'vice_skip_speed', label: '副箕斗提升速度' },
    { value: 'vice_skip_pos', label: '副箕斗提升位置' },
    { value: 'stator_current', label: '定子电流' },
    { value: 'excitation_current', label: '励磁电流' },
    { value: 'incoming_voltage', label: '进线电压' },
    { value: 'brake_oil_pressure', label: '制动油压' },

    // 运行模式与状态反馈
    { value: 'mode_auto', label: '自动模式' },
    { value: 'mode_semi_auto', label: '半自动模式' },
    { value: 'mode_manual', label: '手动模式' },
    { value: 'mode_repair', label: '检修模式' },
    { value: 'mode_lift_coal', label: '提煤模式' },
    { value: 'mode_heavy_down', label: '重物下放模式' },
    { value: 'mode_light_load', label: '轻载模式' },
    { value: 'status_moving_up', label: '上行状态' },
    { value: 'status_moving_down', label: '下行状态' },
    { value: 'status_slow_up', label: '慢上状态' },
    { value: 'status_slow_down', label: '慢下状态' },
    { value: 'status_stopped', label: '停车状态' },
    { value: 'dir_confirmed', label: '运行方向已确定' },
    { value: 'main_fan_run', label: '主风机运行' },

    // 速度反馈与关键位置节点
    { value: 'speed_fb_half', label: '速度反馈0.5m/s' },
    { value: 'speed_fb_2', label: '速度反馈2m/s' },
    { value: 'speed_fb_4', label: '速度反馈4m/s' },
    { value: 'speed_fb_6', label: '速度反馈6m/s' },
    { value: 'speed_fb_12', label: '速度反馈12m/s' },
    { value: 'pos_1_overwind', label: '1系统过卷点' },
    { value: 'pos_1_stop', label: '1系统停车点' },
    { value: 'pos_1_decelerate', label: '1系统减速点' },
    { value: 'pos_1_monitor_2m', label: '1系统2m/s检查点' },
    { value: 'pos_1_sync_calib', label: '1系统同步校正点' },
    { value: 'skip_1_unload_pos', label: '箕斗1在卸载位' },
    { value: 'pos_2_overwind', label: '2系统过卷点' },
    { value: 'pos_2_stop', label: '2系统停车点' },
    { value: 'pos_2_decelerate', label: '2系统减速点' },
    { value: 'pos_2_monitor_2m', label: '2系统2m/s检查点' },
    { value: 'pos_2_sync_calib', label: '2系统同步校正点' },
    { value: 'skip_2_unload_pos', label: '箕斗2在卸载位' },

    // 核心操作、回路状态与故障报警
    { value: 'loop_safety_closed', label: '总安全回路已合' },
    { value: 'loop_lock_closed', label: '闭锁回路已合' },
    { value: 'loop_stop_closed', label: '停车回路已合' },
    { value: 'handle_speed_zero', label: '速度手柄零位' },
    { value: 'handle_brake_zero', label: '闸手柄零位' },
    { value: 'console_lock', label: '操作台闭锁' },
    { value: 'fault_emergency_stop', label: '操作台急停' },
    { value: 'fault_comm', label: '通讯故障' },
    { value: 'fault_low_voltage', label: '低压故障' },
    { value: 'fault_high_voltage', label: '高压故障' },
    { value: 'fault_motor_overload', label: '电机超载' },
    { value: 'fault_motor_overspeed', label: '电机超速' },
    { value: 'fault_temp_alarm', label: '温度报警综合' },
    { value: 'fault_temp_error', label: '温度故障综合' },
    { value: 'fault_brake_wear', label: '闸瓦磨损' },
    { value: 'fault_brake_deflection', label: '闸盘偏摆' },
    { value: 'fault_skip_jam', label: '卡箕斗故障' },
    ...Array.from({length: 6}, (_, i) => ({ value: `motor_temp_${i+1}`, label: `电机温度${i+1}` })),
    ...Array.from({length: 4}, (_, i) => ({ value: `bearing_temp_${i+1}`, label: `轴承温度${i+1}` }))
  ],
  compressor: [
...
onMounted(() => {
  ...
  filterForm.parameter = 'stator_current'
  const now = new Date()
  const endTime = now
